...
I'm still working on the partitioned version - bugs keep popping up

```{r GLS_partition, eval = FALSE}
n.p = 100; n.part = 5 ## stops working when n.p is larger than 100... 
...
### partition 1
y1 <- rnorm(n.p)
X1 <- as.matrix(ndvi_AK3000[partition[,1], -c(1:6)])
loc1 <- ndvi_AK3000[partition[, 1], c("lat","lng")]
V1 <- fitV(D[partition[,1], partition[,1]],
            spatialcor = r.est$spatialcor, fun = "exponential-power")

### partition 2
y2 <- rnorm(n.p)
X2 <- as.matrix(ndvi_AK3000[partition[,2], -c(1:6)])
loc2 <- ndvi_AK3000[partition[, 2], c("lat","lng")]
V2 <- fitV(D[partition[,1], partition[,1]],
            spatialcor = r.est$spatialcor, fun = "exponential-power")

## cross partition
V12 <- fitV(D[as.vector(partition), as.vector(partition)],
             spatialcor = r.est$spatialcor, fun = "exponential-power")
...
## function
# tmp <- fitGLS.partition_rcpp(X, rel.est, X0, D, r.est$spatialcor, Vfit.fun = meth, npart = 5, mincross = 4, nug.int = c(0,1), nug.tol = tolr)
