...
part_input_df = as.data.frame(cbind(rel.est = rel.est, ndvi_AK3000[, c("land", "lng", "lat")]))
```

Then, we'll run `fitGLS.partition.mc()`, making sure to supply any arguments needed 
by `part_data()` to `fitGLS.partition.mc()`, by name.

`fitGLS.partition.mc()`is a multi-core version of `fitGLS.partition()`.
The syntax is the same for the single-core version but 
has a few extra arguments. Check the documentation with `?fitGLS.partition()` 
for more details and examples.

```{r fitGLS_partition, include = FALSE, eval = FALSE}
