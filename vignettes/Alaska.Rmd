...
This function will be called 4 times (`npart`) and needs accesses a different 
...
Below is a simple user-defined function called `part_func()` that uses our 
partition matrix to subset the data at meets the above requirements.

```{r part function}
## Function to get data partitions (will be passed to GLS_partition)
part_func <- function(part.i, part_mat = part.mat, X.full = mod.mat,
                      y.full = rel.est, location.full = location){
  part = part.mat[, part.i]
  return(list(X = as.matrix(X.full[part, ]), y = y.full[part], coords = location[part, ]))
}
```

Then, we'll run `fitGLS.partition()`, making sure to use our user-defined
"part_func" function. Note that `part_f` can be any function and should be 
adapted to the specific data input. For example the `remotePARTs` function 
`part_csv()` is an alternative to `part_func()` and loads partitions of data
into memory one at a time from a .csv file. Similar functions could be created
for any input type or situation.

```{r fitGLS_partition}
## Partitioned Analysis 
GLS.part = fitGLS.partition(part_f = "part_func", 
...
The results are grouped into 3 categories. (1) statistics for each partition:
