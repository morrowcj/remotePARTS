...
It is important to note, though, that `remotePARTS` can handle the full map 
(and much larger maps).
...
greatest distance between two pixels is 1. Note that because `distm_scaled()`
...
```{r visualize_range, fig.asp = 1, fig.width = 4, out.width="50%"}
curve(covar_exp(x, r = .1), xlab = "distance", ylab = "covar_exp(distance, r)")
...
```{r visualize_nugget, fig.asp = 1, fig.width = 4, out.width="50%"}
nugget <- .3
curve(covar_exp(x, r = .1), xlab = "distance", 
      ylab = "covar_exp(distance, r=0.1)")
curve((1-nugget)*covar_exp(x, r = .1), add = TRUE, col = "red")
legend("topright", legend = c(0, nugget), 
       title = expression(eta), col = c("black", "red"), lty = 1)
```
...
<!---- TODO: Below this point has not been revised yet (4/14/2025) ---->

<!---- TODO: also, the base R plots above need to be replaced with ggplot ---->
...
```{r optimized_nugget}
GLS.opt <- fitGLS(formula = AR_coef ~ 0 + land, data = ndvi_AK3000, V = V.opt, 
