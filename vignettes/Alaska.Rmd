...
Note that, because `optimize_GLS()` does not require time series residuals, 
it is possible to use `optimize_GLS()` for statistical problems involving 
...
performance than using `optimize_GLS`. See `?optimize_GLS()` for more 
