...
ggplot(as.data.frame(t.test), aes(x = colnames(mod.mat), y = coefs)) + 
  geom_point() +
  geom_errorbar(aes(ymin = coefs - se, ymax = coefs + se, col = "t-test"), width = .1) +
  geom_errorbar(aes(ymin = coefs - apply(cfs, 1, sd), 
                    ymax = coefs + apply(cfs, 1, sd), 
                    col = "sd(coef)"), width = .1)
