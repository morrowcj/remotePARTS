...
## Effect of time
head(fm.cls$time.coef)

## effect of previous time point
head(fm.cls$xi.coef)
```
...
We are, however, able to reject this $H_0$ that all land classes have the same effect on NDVI trend for this data set at $\alpha = .05$. 
...
Note that, In the below function, a full distance matrix `Dist` is needed as an input.
For larger data sets, it will not be feasible to include the full matrix. The 
...
the partitioned GLS method. Using the following code will allow you to only use
the relevant parts of the large matrices (e.g. `Dist`) at any given time to save
memory. It also allows for parallelization possibilities. 

First, we will use `sample_partitions()` to get a sample of the pixels to use 
for each subset. The result is a matrix with columns containing pixel IDs. 
Each column corresponds to a different partition.
...
```{r}
# optimize_nugget(mod.mat, V, rel.est)
# nugget.fit(rel.est ~ 0 + mod.mat, data.frame(rel.est, mod.mat), V)

# mod.mat
# V
# rel.est

partition = part.mat[, 1]

y.tmp = rel.est[partition]
X.tmp = mod.mat[partition, ]
D.tmp = D[partition, partition]
V.tmp = V[partition, partition]

loc.tmp = location[partition, ]

D.tmp2 = geosphere::distm(loc.tmp)/1000
all.equal(D.tmp, D.tmp2)
V.tmp2 = fitV(D.tmp2, r.est$spatialcor, "exponential-power")
all.equal(V.tmp, V.tmp2)

optimize_nugget(X.tmp, V.tmp, y.tmp, debug = TRUE)
# fitNugget(X.tmp, V.tmp, y.tmp)
# nugget.fit(y.tmp ~ 0 + X.tmp, data = data.frame(y.tmp, X.tmp), V.tmp)
```
...
for(part in seq_len(ncol(part.mat))){
  ...
  part.results[[part]] <- GLS_worker(y.sub, X.sub, V.sub, X0.sub,
                                     save_xx = TRUE)
  # part.results[[part]] = fitGLS2(formula = y.sub ~ 0 + X.sub,
  #                                V = V.sub,
  #                                nugget = NULL, save_xx = TRUE) # slower
}
```
...
(t.test = remoteSTAR::correlated.t(coef = coef.means, part.SEs = part.SEs, rcoef = rcoef.mean, 
                   df2 = df2, npart = ncol(part.mat)))
```

`fitGLS_partition()` **WILL** (eventually) do all of this behind the scenes but 
it may be important when analyzing extremely large data sets to know how the 
...
ggplot(data.frame(t.test, var = factor(colnames(mod.mat), levels = colnames(mod.mat))), 
  ...
  geom_errorbar(aes(ymin = coef - se*t.crit, 
                    ymax = coef + se*t.crit, 
                    col = "correlated t-test", ), position = position_nudge(x = -.025),
                width = .1) +
  geom_errorbar(aes(ymin = coef - apply(cfs, 1, sd)*t.crit, 
                    ymax = coef + apply(cfs, 1, sd)*t.crit, 
                    col = "sd(coef)"),
...
## Tony's code
# source("../inst/tinytest/tony-og-code.R")
Z = GLS.partition.data(formula = "rel.est ~ 0 + land", 
                       formula0 = "rel.est ~ 1",
                       data = dat.map, 
                       spatialcor = r.est$spatialcor, 
                       spatial.autocor.FUN = "exponential-power",
                       est.nugget = TRUE, partition = part.mat)
