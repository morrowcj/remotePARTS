...
Use `help("ndvi_AK)` and `help("ndvi_AK3000")` to see documentation of these 
datasets.
...
* there is no temporal dependence in $\delta(t)$ (i.e., $\delta(t_1)$ is 
independent of $\delta(t_2)$ except when $t_1$ = $t_2$).
...
* the error term $\gamma$ is spatially autocorrelated according to $\Sigma$: $\gamma \sim N(0, \Sigma_gamma)$
...
If the parameters that govern the spatial autocorrelation $\Sigma_gamma$ are known,
...
covariance function (`covar_FUN`), a list of spatial parameters `covar.pars` including our optimized 
range parameter, and `nugget`. If `nugget` is specified, this value is used for the calculations, while
if `nigget` = `NA` (the default) it is estimated for each partition separately.
