...
#'
#' @examples
AR.reml <- function(formula, data = list(), par, x, u){
  AR.reml.funct <- function(par = par, x = x, u = u){
    b <- par
    n.obs <- length(x)
    q <- dim(u)[2]
    B <- diag(n.obs)
  ...
  return(list(beta = beta, b = b, MSE = MSE, s2beta = s2beta, Pr = Pr,
              logLik = logLik))
}
...
#'
#' @examples
simX <- function(formula, data = data.frame(rep(1, n)), coef, b, s, Dr = NULL,
                 t.scale, n, n.obs, n.burn, seed = 0){
  set.seed(seed=seed)
  mf <- model.frame(formula = formula, data = data)
  u <- model.matrix(attr(mf, "terms"), data = mf)
  if (!is.matrix(coef))
