...
#' \code{covar_pars} are given instead.
...
#' @param save_xx logical: should information needed for cross-partition
#' comparisons be returned?
#' @param save_invchol logical: should the inverse of the Cholesky matrix be
#' returned?
#' @param LL_only logical: should calculations stop after calculating parital
#' log-likelihood?
#' @param no_F logical: should F-test calculations be made?
...
#' \code{V} internally
#' @param covar_pars an optional named list of parameters passed to \code{covar_FUN}
...
#' null model.
#'
#' \code{save_xx} is included to allow for manually conducting a partitioned
#' GLS analyses. Because most users will not need this feature, opting instead
#' to use \code{fitGLS_parition()}, \code{save_xx = FALSE} by default.
#'
#' Similarly, \code{save_invchol} is included to allow for recycling of the
...
#'
#' \code{LL_only = TRUE} will return only the partial log-likelihood, which can
#' minimized to obtain the maximum likelihood for a given set of data.
#'
#' If \code{no_F = TRUE}, then the model given by \code{formula} is not compared
#' to the model given by \code{formula0}.
#'
#' If \code{V} is not provided, it can be fit internally by specifying all of
#' \code{coords}, \code{distm_FUN}, \code{covar_FUN}, and \code{covar_pars}.
...
#' matrix with \code{covar_FUN} and parameters given by \code{covar_pars}.
#'
...
#' \code{LL_only = FALSE}. The list contains at least the following elements:
#'
#' \describe{
...
#' }
#'
#' If \code{no_F = FALSE}, the following elements, corresponding to the null
...
#' }
#'
#' An attribute called also set to \code{"no_F"} is set to the value of
#' argument \code{no_F}, which signals to generic methods how to handle the output.
#'
#' If \code{LL_only = TRUE}, a single numeric output containing the partial
...
#' (GLS.F = fitGLS(CLS_coef ~ 0 + land, data = df, V = V, no_F = FALSE))
#'
#' ## find ML nugget
#' fitGLS(CLS_coef ~ 0 + land, data = df, V = V, no_F = FALSE, nugget = NA)
#'
#' ## calculate V internally
#' coords = cbind(df$lng, df$lat)
#' fitGLS(CLS_coef ~ 0 + land, data = df, LL_only = FALSE, coords = coords,
#'        distm_FUN = "distm_scaled", covar_FUN = "covar_exp", covar_pars = list(range = .01))
#'
#' ## use inverse cholesky
#' fitGLS(CLS_coef ~ 0 + land, data = df, invCholV = invert_chol(V))
#'
#' ## save inverse cholesky matrix
#' invchol = fitGLS(CLS_coef ~ 0 + land, data = df, V = V, save_invchol = TRUE)$invcholV
#'
#' ## re-use inverse cholesky instead of V
#' fitGLS(CLS_coef ~ 0 + land, data = df, invCholV = invchol)
#'
#' ## Log-likelihood (fast)
#' fitGLS(CLS_coef ~ 0 + land, data = df, V = V, LL_only = TRUE)
#'
#' @export
fitGLS <- function(formula, data, V, nugget = 0, formula0 = NULL, save_xx = FALSE,
                   save_invchol = FALSE, LL_only = FALSE, no_F = TRUE,
                   coords, distm_FUN ,covar_FUN, covar_pars, invCholV,
                   ...){
  ...
  if (missing(V) & !use_invCholV) {
    if (any(missing(coords), missing(distm_FUN), missing(covar_FUN))) {
      stop("If V or invCholV are not provided, then coords, distm_FUN, covar_FUN, and covar_pars are needed to calculate V")
    }
    ...
    pars = append(list(V), covar_pars) # pars list
  ...
  if (!all(check_posdef(V))) {
    if (LL_only){
      ...
      return(NA) # return NA for LL function
    }
  ...
  GLS <- .Call(`_remotePARTS_fitGLS_cpp`, X, V, y, X0,
               nugget, save_xx, save_invchol, LL_only, no_F,
  ...
  if(LL_only){
    return(unlist(GLS$LL))
  }

  # pvalues
  GLS$pval_t <- 2 * pt(abs(GLS$tstat), df = GLS$df_t, lower.tail = F)
  if(!no_F){GLS$pval_F <- pf(GLS$Fstat, df1 = GLS$df_F[1], df2 = GLS$df_F[2], lower.tail = F)}
